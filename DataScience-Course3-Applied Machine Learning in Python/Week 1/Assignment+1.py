...
def answer_one():
    ...
    return pd.DataFrame(np.c_[cancer['data'], cancer['target']], columns=cancer['feature_names'].tolist() + ["target"])
...
def answer_two():
    ...
    return [len(cancerdf[cancerdf['target'] == 1]), len(cancerdf[cancerdf['target'] == 0])]
