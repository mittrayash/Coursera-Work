...
import datetime
print(
    datetime.datetime.fromtimestamp(1510785680).strftime('%Y-%m-%d %H:%M:%S')
)
...
'''  Q - DIFF. Between FIT_TRANSFORM() and TRANSFORM()
...
plt.scatter(X_C2[:, 0], X_C2[:, 1], c=y_C2, # Note that here we plot X_C2[:, 0] and X_C2[:,1] since y_C2 is binary class with 0/1 only and is not meant for plotting. It's just a classification variable.
           marker= 'o', s=50, cmap=cmap_bold)# So we use y_C2 for color
plt.show()
